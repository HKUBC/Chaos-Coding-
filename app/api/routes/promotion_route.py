...
def create_item_promotion(promo_id: str, item_id: str, discount: float):
    ...
    promo_service.item_promotions.append(promo)
...
def delete_item_promotion(promo_id: str):
    before = len(promo_service.item_promotions)

    promo_service.item_promotions = [p for p in promo_service.item_promotions
                                        if p.promo_id != promo_id]
    
    after = len(promo_service.item_promotions)
...
def create_order_promotion(promo_id: str, promo_type: str, value: float, min_total: float = 0):
    ...
    promo_service.order_promotions.appeend(promo)
...
def delete_order_promotion(promo_id: str):
    before = len(promo_service.order_promotions)

    promo_service.order_promotions = [p for p in promo_service.order_promotions
                                        if p.promo_id != promo_id]
    
    after = len(promo_service.order_promotions)
...
@router.get("/item/{item_id}/apply")
def apply_item_promotion(item_id: str):
    ...
    final_price = promo_service.apply_item_promotion(item)
    return {
        "item_id": item_id,
...
def apply_order_promotion(order_id: str):
    ...
    final_total = promo_service.apply_order_promotion(order, og_total)
    return {
        "order_id": order_id,
