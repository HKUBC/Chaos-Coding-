...
def test_api_get_delivery_valid_order():
    ...
    with patch("app.api.routes.delivery_route.order_service") as mock_order_service, \
        ...
        response = client.get("/deliveries/1d8e87M")

        assert response.status_code == 200

def test_api_get_delivery_invalid_order():
    response = client.get("/deliveries/invalid_order_id")
