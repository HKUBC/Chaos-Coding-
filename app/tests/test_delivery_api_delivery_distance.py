...
def test_api_get_delivery_distance_valid_order():
    with patch("app.services.delivery_service.repo") as mock_repo:
        ...
        with patch("app.services.order_service.repo") as mock_order_repo:
            ...
            response = client.get("/deliveries/1d8e87M/distance")
            assert response.status_code == 200

def test_api_get_delivery_distance_invalid_order():
    with patch("app.services.delivery_service.repo") as mock_repo:
        mock_repo.deliveries = {}
        mock_repo.get_delivery_distance.return_value = None

        response = client.get("/deliveries/invalid_order_id/distance")
